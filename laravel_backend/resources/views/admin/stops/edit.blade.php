...
@section('content')
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-blue-600 mb-6">Edit Bus Stop</h1>

        <form action="{{ route('admin.stops.update', $stop->id) }}" method="POST" class="bg-white rounded shadow p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name"
                    ...
                    required value="{{ old('name', $stop->name) }}">
                @error('name')
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" step="any" name="latitude" id="latitude"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required value="{{ old('latitude', $stop->latitude) }}">
                    @error('latitude')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="any" name="longitude" id="longitude"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required value="{{ old('longitude', $stop->longitude) }}">
                    @error('longitude')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
